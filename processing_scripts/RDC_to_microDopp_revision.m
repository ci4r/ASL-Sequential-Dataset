function [] = RDC_to_microDopp( RDC, fOut, cfar_bins )
      ...
      %% MTI Filter (not working)
       ...
       %% MTI v2
            [b,a]=butter(4, 0.02, 'high'); %  4th order is 24dB/octave slope, 6dB/octave per order of n
...
%             rBin = min(cfar_bins(1)):1+max(cfar_bins(2)); %covid 18:30, front ignore= 7:nts/2, %lab 15:31 for front
      ...
      %% cfar bins
        ...
        colormap(gray);
        frame2 = frame2im(getframe(gca));
        imwrite(frame2,[fOut(1:end-4) 'gray.png']);
        close all
